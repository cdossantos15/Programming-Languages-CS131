...
class ServerProtocol(Protocol):
    ...
    async def send_IAMAT(self, message):
        ...
        if not check_location(args[2]):
            self.respond("? " + message)
            return
        AT_message = "AT " + server_name + " " + time_skew(time_sent) + " " + " ".join(args[1:]) + "\n"
        # send At message, then respond
    ...
    async def send_WHATSAT(self, message):
        ...
        try:
            ...
            self.respond("? " + message)
            return
        if upper_bound > 20 or radius_km > 50:
            self.respond("? " + message)
            return
        ...
        json_msg['results'] = json_msg['results'][:upper_bound]
    ...
    async def send_AT(self, message):
        ...
        # update the locations of client
...
def time_skew(time_sent):
    diff = time.time() - time_sent
    if diff > 0:
        return "+" + str(diff)
    return str(diff)
...
if len(sys.argv) != 2:
    print("Invalid command. Check argument list.\n")
    exit(1)
